...
// ─────────────────────────────────────────────
// Carga todas las series desde la API
// ─────────────────────────────────────────────
async function cargarSeries() {
  try {
    ...
    loading.style.display = 'none';
    errorDiv.textContent = 'Error al cargar las series. Verifica que el servidor este corriendo.';
...
// ─────────────────────────────────────────────
// Crea una tarjeta individual para cada serie
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Carga detalles, episodios y actores en paralelo
// ─────────────────────────────────────────────
async function mostrarDetalles(serieId) {
  try {
    const [serieResponse, actoresResponse] = await Promise.all([
      fetch(`${API_URL}/series/${serieId}`),
      fetch(`${API_URL}/series/${serieId}/actores`)
    ]);
    ...
    if (serieData.length === 0) {
      modalBody.innerHTML = '<p>No se encontro informacion.</p>';
    } else {
